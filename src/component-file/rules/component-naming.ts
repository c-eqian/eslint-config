...
import { COMPONENT_EXTNAME, DEFAULT_PATH_PATTERN, NAMING_MAP } from '../constants';
...
export default {
    meta: {
        type: 'layout',
        docs: {
            description: 'Enforce PascalCase for component filenames with .vue or .tsx extension',
        ...
        return {
            Program(node: AST.Program) {
                ...
                for (const [filenamePattern, targetPattern] of Object.entries(rules)) {
                    if (micromatch.isMatch(filenameWithPath, filenamePattern) &&
